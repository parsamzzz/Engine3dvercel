...
router.get("/status/:service/:taskId", async (req, res) => {
  ...
  try {
    ...
    // ✅ حذف فیلدهای خاص برای هر سرویس
    const cleanData = { ...statusResp.data };
    if (cleanData.data?.data) {
      if (serviceType === "runway" || serviceType === "runway_extend") {
        delete cleanData.data.data.generateParam;
      } else if (serviceType === "aleph") {
        delete cleanData.data.data.paramJson;
      }

      if (cleanData.data.data.videoInfo) {
        delete cleanData.data.data.videoInfo;
      }
    }
